...
### Editor improvements split (2026-09-20)

The independent editor branch retains the localization fixes. CK3 ranking output is identical to the source branch across 1,400 sampled positions, excluding timing. Frequency regeneration is byte-identical across the split and matches the shipped table apart from its generation date.

The `source-before-editor-split` and `editor-improvements-split` history rows use the Cultivation workspace. Both retain 481,510 definitions, 559,734 shared identifiers and a 271 MB post-GC heap. The single runs measured 26,992/14,015 ms to index and 149/195 ms for completion after a save. Other checks ran concurrently, and disk caches were not controlled. These rows check that the index contents and memory use survive the split; they do not establish a performance change.
