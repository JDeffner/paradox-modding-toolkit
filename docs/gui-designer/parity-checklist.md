...
(S07). Six rows stayed open, and they are the honest residue rather than an oversight: three
DISPUTED (L07c, L13e, L23) and three deferred for want of a measurement or a preview-UX
decision (L01f, L06c, L11b). L17c is a seventh row the merge did not close, but it is a
...
| L25 | An EMPTY `container` collapses to 0: a fixed `size` will not hold it open | `layout/container-measurability.gui` | **done (G2)**: `container` (and a datamodel `item`) content-size ALWAYS, so an explicit `size` no longer holds an empty one open. Asserted by `guiLayoutMerge.test.ts` "L25: an EMPTY container collapses to 0". Implemented BROADER than the bullet, which measured the EMPTY case only: a non-empty `container` now drops its authored `size` too. The narrow alternative is to keep an explicit `size` once the container has children. The broad reading is what the bullet's "it sizes to content" says, and it costs nothing in vanilla (no `container = { … }` block in the 373 game files writes its own top-level `size`; 431 blocks scanned 2026-08-01), so the difference only shows on hand-authored or mod files, or through a template that supplies one. Owner to confirm the broad reading or ask for the narrow one |
...
| W10 | Refusal honesty: `position` on a box child, `size` on a content-sized type (hbox/vbox/flowcontainer), `size` on a child expanding on BOTH axes; one expanding axis writes and names the axis the box owns; the guard must NOT fire outside a layout container | `writer/refusal-shapes.gui` | **done (G1 stage 4)**: `positionIgnoredReason` / `sizeIgnoredReason` in `packages/server/src/gui/sourceEditService.ts`, six cases in `guiSourceEditService.test.ts` "position and size guards (W10, S09)", each on the shape that must refuse AND the neighbour that must not. Classes come from the engine's own `widgetClassOf`, resolved through `typeBaseChain`, never re-derived. The position half takes the DISPUTED side of L23 deliberately: the Studio measured the box dropping a child's position, spec.md is silent, and this engine's own note calls its offset unmeasured, so the writer refuses to write a property the game most likely ignores while the engine keeps rendering it. Settling L23 the other way removes the guard |
...
| S02 | Single-entry rewrite leaves every OTHER byte identical, and the file still parses to the same root count | **done (G1 stage 2)**: `guiSourceEdit.test.ts` "over the fixture corpus, a rewrite changes exactly its value span" (39 files, 367 rewrites) and "a single-entry rewrite is byte-identical over the vanilla gui tree" (373 files, 40,585 rewrites, dev-paths-gated). Zero drift on both; baselines in §G |
...
|---|---|---|---|---|
| Fixture corpus (`test/fixtures/gui/`) | 39 | 367 | 367 | **0** |
...
asserts the rest of the file is byte-identical, so 367 is the number of fixture bodies
that hold a property (of 779 bodies), and 40,585 the vanilla number. The second half of
...
|---|---|---|---|---|---|---|---|
| Fixture corpus (`test/fixtures/gui/`) | 39 | 59 | 2 | 399 | 394 | 227 | **0** |
