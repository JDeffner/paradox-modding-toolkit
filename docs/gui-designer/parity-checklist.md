...
| W25 | Single-line insert/delete are exact inverses, including the separator space (the `{ a  }` accumulation bug) | `writer/single-line-bodies.gui` | **G1** for the inverse; the insert half is stage 2's (`guiSourceEdit.test.ts` "a single-line body stays single-line" and "an empty single-line body gets a spaced entry (W25)"), and the delete it must invert is stage 3's |
