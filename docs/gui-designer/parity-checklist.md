...
document for G1 (writer rebuild) and G2 (layout merge). The current engine rules are in [spec.md](spec.md), and the host and webview responsibilities are in the [GUI editor README](../../packages/vscode/src/webviews/guiEditor/README.md).
...
| **done (G3.n)** | Landed in that stage of the G3 webview editor (docs/gui-designer/g3-plan.md), with the citing test named. Rows arrive here when the answer is a PRESENTATION decision the canvas owns rather than a measured engine rule. |
...
L25 owner question. Of the three that were deferred for want of a measurement or a
preview-UX decision, L11b closed in G3.1 on the canvas that owns it, leaving L01f and
L06c, both of which still need a measurement before they can be code. L17c remains a
deliberate divergence that renders the same pixels rather than a gap (§E).
...
| L11b | Content-sizing runs only when the content is statically MEASURABLE; `{ 0 0 }` children, a child of an unknown type, a `datamodel`, or a binding-valued `visible` all make it unmeasurable, and the preview keeps a visible default rather than collapsing | `layout/container-measurability.gui` | **done (G3.1)**, on the canvas, exactly as the deferral said: the ENGINE still invents nothing and every rect is unchanged, and the RENDERER draws a dashed GHOST_BOX (40x40, at GHOST_OPACITY) for a content-sized widget that has children yet measured to nothing. Scoped so it can only ever add an affordance, never overwrite an answer: content-sized keys only (a `widget`/`button` at 0x0 is measured, B4-T1), children required (a childless collapse is L25's measured behavior), and an axis the engine DID measure is kept. The datamodel and binding cases need no box, since neither collapses. 40 is a preview affordance with nothing behind it, stated as such in `app/scene.ts`; the known limit is that an instance of a custom `type x = container` gets no box because the wire carries the source key, not the resolved class. Asserted by `packages/vscode/test/guiEditorScene.test.ts` "L11b: unmeasurable content renders as a dashed ghost" and recorded in the scene baseline `packages/vscode/test/fixtures/gui-scene.baseline.txt` |
...
| L30 | Reported percentage-width hazard inside a content-sized vbox | none | **unverified, requires game probe**: the crash claim comes from Studio's GUI007 rule and has not been reproduced. Compare content-sized and explicitly sized parents before adding an authoring warning. The editor currently accepts the size; the documentation is not evidence that this limitation was accepted. |
...
| S07 | Headless rect dump over a corpus (`--render-gui` equivalent), diffed numerically before and after a change | **done (G2)**: `guiLayoutMerge.test.ts` "S07: rect dump over the fixture corpus" dumps every rect of every `layout/` fixture against `test/fixtures/gui/layout-rects.baseline.txt`, recorded 2026-08-01 over the merged engine. Re-record with `PX_WRITE_GUI_RECT_BASELINE=1` so an intended change lands as a numeric diff in review. The Studio's 126-rect test-mod dump stays unreproducible here by design (§G). G3.1 added the RENDERER-side companion, `packages/vscode/test/guiEditorScene.test.ts`, which dumps the webview's draw list (`label x y w h fillKind [flags]`) against `packages/vscode/test/fixtures/gui-scene.baseline.txt` (`PX_WRITE_GUI_SCENE_BASELINE=1` to re-record) and asserts, per fixture and in draw order, that a scene rect EQUALS the engine's rect. That equality is what keeps the canvas from acquiring layout opinions |
| S08 | Headless app smoke driving the REAL editor through select → add → duplicate → delete → undo → redo, asserting one document edit per batch, selection surviving the re-parse, and the file byte-identical after undoing the run (`--gui-edit-smoke` equivalent) | **partly done (G3.2)**, the harness and the selection half: `packages/vscode/test/guiEditorHarness.ts` boots the REAL app bundle (esbuild, the same entry the compile chain ships) in jsdom against the same markup `html.ts` serves the panel, with a stubbed host that speaks `messages.ts` and replays the real server. `guiEditorSmoke.test.ts` drives clicks through it: select, Alt-cycle, Esc, reveal, the inspector rows, and SELECTION SURVIVING THE RE-PARSE across an insert above it, a property edit under it and a delete of it. jsdom is not a browser, so exactly three things are stubbed and all are plumbing (a no-op 2d context, a fixed viewport, `scrollIntoView`); Playwright stays unneeded. The edit half (add/duplicate/delete, one document edit per batch, byte-identity after undo) needs the gestures, and lands on this harness in G3.3 |
...
| X08 | **Property provenance**: a local value reports no origin, a chain value names the type that supplies it, a deep-chain value names the ancestor, a `using` mixin names the mixin, a local override still reveals the shadowed chain value, the type chain walks to the built-in root, and the inherited listing excludes locally-set keys | **done (G3.2)** except the shadowed value: `paradox/guiWidgetInfo` (`gui/widgetInfo.ts`) answers one widget's effective properties, each with the chain it was spliced through, innermost first. It goes through the engine's OWN expansion (`expandWidgetWithOrigins`, one implementation, so a row cannot disagree with the rendering) and addresses the widget the way the writer does (`findWidgetAtLine`), so an inspector row and a `setProperties` target are the same bytes. Asserted by `guiWidgetInfo.test.ts`: empty origin = local, `type px_card`, a derived type over its base, `template PxDeco`, a template used inside a type reporting both steps, a cross-file template through the store, and last-in-wins dropping the shadowed row. The one sub-claim left is REVEALING that shadowed value: it is the "overrides X" click-through the consolidation doc puts in G5, and the inspector shows only what the engine used until then |
| X09 | **GuiLinter** findings (the GUI001/007/008/009/010/015 codes the quirk list cites) | **dropped** from G1/G2: no `.gui` linter exists here and none is in the G-plan. AGENTS.md's "deep validation belongs to ck3-tiger" is the standing reason; the quirk knowledge lives in `spec.md` instead |
...
- **L11b** (the measurability guard) needs a preview default SIZE that no measurement
  pins; it moves to G3 with the canvas. (Closed there on 2026-08-02, G3.1: the default is
  a dashed 40x40 ghost box drawn by the renderer, and no engine rect moved.)
...
## F. Correspondences with `spec.md`
...
`spec.md` under "Studio-verified engine behaviors", each labeled with its
