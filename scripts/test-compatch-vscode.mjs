...
const testing = join(root, ".local/testing");
await mkdir(testing, { recursive: true });
const scratch = await mkdtemp(join(testing, "compatch-editor-"));
...
exports.run = async () => {
  ...
  assert.equal(tab.input.modified.scheme,"px-compatch","target is read-only");
  ...
  assert.ok(target.getText().includes("id_override_priority"),"new version snapshot has content");
