...
export async function run() {
  ...
  assert.ok(tab.label.startsWith("Old Game Version"), "roles lead the tab title");
  assert.equal(tab.input.original.scheme, "px-compatch");
  assert.equal(tab.input.modified.scheme, "px-compatch", "target is a read-only snapshot");
