...
export async function run() {
  ...
  assert.equal(tab.input.modified.fsPath, path.join(mod, eventPath));
  assert.ok(tab.label.startsWith("New Game Version"));
  const editor = vscode.window.visibleTextEditors.find(
    (e) => e.document.uri.fsPath === path.join(mod, eventPath)
  )!;
  assert.equal(
    editor.selection.start.line,
    event.entry.sites.A[0].line - 1,
    "event navigation reaches the selected event"
  );
