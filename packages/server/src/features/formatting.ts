/**
 * Conservative formatter (rework plan Phase 5): indentation only — one client
 * indentation unit per brace depth, computed from the lexer so strings and comments can never fool
...
export function provideFormattingEdits(document: TextDocument, options?: FormattingOptions): TextEdit[] {
  ...
  for (let line = 0; line < lines.lineCount; line++) {
    ...
    const target = indentation.repeat(Math.max(0, openBefore[line] - closersAtStart[line]));
