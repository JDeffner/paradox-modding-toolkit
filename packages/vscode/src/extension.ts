...
import { convertImagesCommand } from "./imageConvert";
...
import { registerEncodingFix } from "./encodingFix";
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  registerEncodingFix(context, () => cfg);
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("px.openInfoDocs", (uri?: vscode.Uri) => openInfoDocsCommand(cfg, uri)),
    vscode.commands.registerCommand("px.openVanillaExamples", (uri?: vscode.Uri) =>
      openVanillaExamplesCommand(uri)
    ),
    vscode.commands.registerCommand("px.convertImages", (arg?: vscode.Uri, multi?: vscode.Uri[]) =>
      convertImagesCommand("images", arg, multi)
    ),
    vscode.commands.registerCommand("px.convertDdsToImage", (arg?: vscode.Uri, multi?: vscode.Uri[]) =>
      convertImagesCommand("fromDds", arg, multi)
    ),
    vscode.commands.registerCommand("px.openDdsPreview", (arg?: vscode.Uri) => {
      const uri = arg ?? vscode.window.activeTextEditor?.document.uri;
      if (uri) return vscode.commands.executeCommand("vscode.openWith", uri, "px.ddsPreview");
    }),
    vscode.commands.registerCommand("px.convertToDds", (arg?: vscode.Uri, multi?: vscode.Uri[]) =>
      convertImagesCommand("toDds", arg, multi)
    ),
