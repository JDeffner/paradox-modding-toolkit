...
export function queueState(entry: Entry, session: Session): "pending" | "manual" | "reviewed" | "skipped" {
  const review = session.reviews[entry.id];
  if (review?.fingerprint === fingerprint(entry)) return review.status;
  if (
...
export function mergeEligible(entry: Entry): boolean {
  return (
