...
export interface Session {
  version: 1;
  /** Absent on saved two-source comparisons. In update mode A is the editable mod. */
  mode?: "game-update";
  gameId: string;
...
export async function validateRoots(session: Session): Promise<void> {
  ...
  if (session.mode === "game-update") {
    if (!session.roots.A || !session.roots.B || !session.roots.base)
      throw new Error("Choose Mod, New Game Version and Vanilla folders.");
    if ((await fs.realpath(session.roots.A)) !== output)
      throw new Error("A game update edits the original Mod folder.");
    ...
    for (let i = 0; i < sources.length; i++)
      for (let j = i + 1; j < sources.length; j++)
        if (contains(sources[i], sources[j]) || contains(sources[j], sources[i]))
          throw new Error("Mod, Vanilla and New Game Version must be separate, non-overlapping folders.");
  }
  const sources =
    session.mode === "game-update" ? [session.roots.B, session.roots.base] : Object.values(session.roots);
