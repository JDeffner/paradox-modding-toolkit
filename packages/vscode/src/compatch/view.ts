...
type CommandArgument = (Row & CompatchSources) | { entryId: string; sessionId?: string };
...
class CompatchView implements vscode.TreeDataProvider<Row>, vscode.TextDocumentContentProvider {
  ...
  constructor(
    ...
    const commands: Record<string, (row?: Row & CompatchSources) => Promise<unknown>> = {
      ...
      "px.compatchReplacement": (row) =>
        row && "entry" in row ? this.replacementIntent(row.entry) : Promise.resolve(),
      ...
      "px.compareCompatch": async (row) => {
        if (row && "entry" in row && this.session && !this.busy) {
          await this.openEntry(row.entry);
        }
      },
      ...
      "px.compatchModChanges": async (row) => {
        if (row && "entry" in row) await this.compareMod(row.entry, "base");
      },
      ...
      "px.compatchEditorActions": async () => {
        ...
        const entry =
          ...
          (this.session?.mode !== "game-update" ||
            needsGameUpdate(file) ||
            isIntentionalReplacement(file, this.session) ||
            (candidates.length === 0 && file.sites.A.length > 0))
            ? file
            : candidates.length === 1
              ? candidates[0]
              : (
    ...
    for (const [id, action] of Object.entries(commands)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(id, async (argument?: CommandArgument) => {
          try {
            let row: (Row & CompatchSources) | undefined;
            if (argument && "entryId" in argument) {
              const entry = this.inventory.entries.get(argument.entryId);
              if (!entry || argument.sessionId !== this.session?.id)
                throw new Error("This comparison changed. Select an entry from the current Compatch view.");
              row = this.rowFor(entry);
            } else row = argument;
            if (
              this.session &&
  ...
  getTreeItem(row: Row): vscode.TreeItem {
    ...
    item.id = entry.id;
    const replacement = this.session && isIntentionalReplacement(entry, this.session);
    item.contextValue = `px.compatchEntry${this.session?.mode === "game-update" ? ".update" : ""}${entry.kind === "file" && entry.sites.A.length === 1 ? ".file" : ""}${mergeEligible(entry, this.session) && this.session?.mode === "game-update" ? ".merge" : ""}${entry.kind === "localization" && !replacement ? ".key" : ""}${replacement ? ".replacement" : ""}${!entry.sites.B.length && entry.kind === "file" ? ".missing" : ""}${this.session?.relocations?.[entry.id] ? ".linked" : ""}${this.session?.reviews[entry.id] ? ".reviewed" : ""}`;
    ...
    item.tooltip = [
      ...
      ...(replacement
        ? [
            "Preserves mod content during updates. Target validation is separate and still checks the whole mod.",
          ]
        : []),
      ...
      this.session?.mode === "game-update"
        ? "Select to review Old Game Version against New Game Version. Game differences are not required mod edits. Row actions offer read-only mod references, replacement intent and a separate merge preview."
        : "Select to compare. Use the row actions to edit a result or record your review.",
    ].join("\n");
    // Tree command arguments cross the extension-host boundary. Keep source text in the inventory.
    item.command = {
      command: "px.compareCompatch",
      title: "Compare changes",
      arguments: [{ entryId: entry.id, sessionId: this.session?.id }],
    };
    return item;
  }
  ...
  private entryStatus(entry: Entry): string {
    if (this.session && isIntentionalReplacement(entry, this.session))
      return `Intentional replacement${this.hasDirtyDocument(entry) ? " (unsaved edits)" : ""}`;
  ...
  private async refresh(candidate = this.session): Promise<void> {
    ...
    this.busy = true;
    const previousMessage = this.view.message;
    this.view.message = "Scanning compatch sources...";
    ...
    try {
      ...
      this.view.message = previousMessage;
      throw error;
    } finally {
      this.busy = false;
    }
  ...
  private async freshEntry(entry: Entry, rejectChanged = false): Promise<Entry> {
    ...
    const revision = this.inputRevision;
    const dirtyCount = this.dirtyInputs.size;
    const inputs = [
      ...entryInputs(entry),
    ...
    if (affected.size || dirtyCount !== this.dirtyInputs.size) this.changed();
    if (
      rejectChanged &&
  ...
  private updateSummary(): void {
    ...
    const counts = { pending: 0, manual: 0, reviewed: 0, skipped: 0, replacement: 0 };
    ...
    this.view.message = `${counts.pending} pending · ${counts.manual} need manual review · ${counts.reviewed} reviewed · ${counts.skipped} skipped${counts.replacement ? ` · ${counts.replacement} intentional replacements` : ""}. ${this.dirtyInputs.size ? `${this.dirtyInputs.size} changed inputs. Refresh Changed Sources. ` : ""}${this.inventory.issues.length} scan notes. ${metaFor(this.session.gameId).compatch ? `Files and definitions overlap. Localization: ${this.session.localizationLanguage}.` : "File comparison only."} ${this.session.validation ? `Target validation${this.session.validation.stale ? " (outdated)" : ""}: ${this.session.validation.summary}` : "Target validation has not run."}`;
  }
  ...
  private async chooseQueue(): Promise<void> {
    const pick = await vscode.window.showQuickPick(
      [
        ...
        ...(this.session?.mode === "game-update"
          ? [{ label: "Intentional replacements", value: "replacement" }]
          : []),
      ],
      { title: "Review queue" }
    );
  ...
  private async openEntry(entry: Entry): Promise<void> {
    if (this.session?.mode === "game-update") await this.compare(entry, "base", "B");
  ...
  private async replacementIntent(entry: Entry): Promise<void> {
    ...
    if (
      ...
      this.busy ||
      entry.kind !== "file" ||
      entry.sites.A.length !== 1
    )
      return;
    if (!this.stateFile) throw new Error("Open a workspace folder to save replacement intent.");
    const relative = entry.sites.A[0].path;
    const matching = replacementRulesForPath(session, relative);
    const choices: { label: string; description: string; rule: ReplacementRule; remove?: boolean }[] = [];
    if (!matching.length)
      choices.push({
        label: "Keep this file as an intentional replacement",
        description: relative,
        rule: { kind: "file", path: relative },
      });
    const parts = relative.split("/");
    for (let end = parts.length - 1; end > 0; end--) {
      const folder = parts.slice(0, end).join("/");
      if (matching.some((rule) => rule.kind === "folder" && rule.path === folder)) continue;
      choices.push({
        label: `Keep all mod files in ${folder}/ as intentional replacements`,
        description:
          "Includes subfolders and future mod files. Does not exclude new vanilla files from the game.",
        rule: { kind: "folder", path: folder },
      });
    }
    for (const rule of matching)
      choices.push({
        label: `Remove replacement rule: ${rule.path}${rule.kind === "folder" ? "/" : ""}`,
        description:
          rule.kind === "folder"
            ? "Removes this rule for every mod file in this folder."
            : "Allows content updates for this file again.",
        rule,
        remove: true,
      });
    const pick = await vscode.window.showQuickPick(choices, {
      title: "Replacement intent",
      placeHolder: "Preserve mod content during updates. This does not mark it reviewed or validated.",
      matchOnDescription: true,
    });
    if (!pick || this.session !== session) return;
    const previous = session.intentionalReplacements;
    session.intentionalReplacements = pick.remove
      ? (previous ?? []).filter((rule) => rule.kind !== pick.rule.kind || rule.path !== pick.rule.path)
      : [
          ...(previous ?? []).filter((rule) => rule.kind !== pick.rule.kind || rule.path !== pick.rule.path),
          pick.rule,
        ];
    try {
      ...
      session.intentionalReplacements = previous;
      throw error;
    }
    this.changed();
  }
  ...
  private async coverage(): Promise<void> {
    ...
    for (const rule of this.session.intentionalReplacements ?? [])
      this.log.appendLine(
        `Intentional replacement (${rule.kind}): ${rule.path}. Content merges are blocked; whole-mod target validation still applies.`
      );
    this.log.appendLine(
  ...
  private async openMod(entry: Entry): Promise<void> {
    if (!this.session || this.busy || this.session.mode !== "game-update") return;
    entry = await this.freshEntry(entry);
    const mod = await this.chooseSite(entry, "A");
    if (!mod) return;
    await vscode.window.showTextDocument(vscode.Uri.file(await resultPath(this.session, mod.path)), {
      preview: true,
      selection: new vscode.Range(mod.line - 1, 0, mod.line - 1, 0),
    });
  }

  private async compareMod(entry: Entry, side: Side): Promise<void> {
    if (!this.session || this.busy || this.session.mode !== "game-update") return;
    entry = await this.freshEntry(entry);
    const mod = await this.chooseSite(entry, "A");
    if (!mod) return;
    ...
    // unrelated vanilla event in its former file. References cannot copy game
    // content into the mod through the diff editor's built-in revert controls.
    const text = source
    ...
    const modLabel = doc.isDirty ? "Mod (reference, unsaved edits)" : "Mod (reference)";
    const modUri = await this.snapshot(doc.getText(), modLabel, mod.path);
    await vscode.commands.executeCommand(
      "vscode.diff",
      uri,
      modUri,
      `${this.sideLabel(side)} ↔ ${modLabel} · ${path.basename(mod.path)}`,
      {
        preview: true,
  ...
  private async mergeMod(entry: Entry, apply: boolean): Promise<void> {
    ...
    for (const site of entry.sites.A) assertContentUpdateAllowed(this.session, site.path);
    if (!mergeEligible(entry, this.session))
      throw new Error(
    ...
    if (!apply || merged.conflicts) {
      ...
      this.resolutions.set(result.toString(), {
        ...
        entryId: entry.id,
        relativePath: entry.sites.A[0].path,
        target,
        document: doc,
        version,
        original,
  ...
  private async applyResolved(): Promise<void> {
    ...
    assertContentUpdateAllowed(this.session, resolution.relativePath);
  ...
  private async publishResolved(
    ...
    const relative = resolution.relativePath;
    assertContentUpdateAllowed(session, relative);
  ...
  private async createLocalization(entry: Entry, side: Side, apply = false): Promise<void> {
    ...
    assertContentUpdateAllowed(session, relative);
    for (const site of entry.sites.A) assertContentUpdateAllowed(session, site.path);
  ...
  private async actions(row?: Row): Promise<void> {
    ...
    if (this.session.mode === "game-update") {
      const choices = [
        {
          ...
          description: "Read-only game changes; applicability to your mod needs review",
          run: () => this.compare(entry, "base", "B"),
        },
        {
          label: "New Game Version ↔ Mod (reference)",
          description: "Read-only comparison; differences can be intentional",
          run: () => this.compareMod(entry, "B"),
        },
        {
          label: "Old Game Version ↔ Mod (reference)",
          description: "Read-only comparison of your mod changes, including unsaved edits",
          run: () => this.compareMod(entry, "base"),
        },
        {
          label: "Open mod file",
          description: "Edit your mod separately from the game comparisons",
          run: () => this.openMod(entry),
        },
      ];
      if (entry.kind === "file" && entry.sites.A.length === 1)
        choices.push({
          label: "Set replacement intent",
          description: "Preserve this file or folder during updates; keep target validation separate",
          run: () => this.replacementIntent(entry),
        });
      if (mergeEligible(entry, this.session))
        choices.push(
          {
            label: "Preview three-way merge",
            description: "Review a text merge proposal; it does not establish compatibility",
            run: () => this.mergeMod(entry, false),
          },
          {
            label: "Apply clean game changes",
            description: "Write a text merge only if Git finds no conflicts; review applicability first",
      ...
      if (
        entry.kind === "localization" &&
        entry.sites.B.length === 1 &&
        !isIntentionalReplacement(entry, this.session)
      )
        choices.push(
          {
