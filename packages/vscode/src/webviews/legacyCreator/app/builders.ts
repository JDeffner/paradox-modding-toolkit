...
export function conditionField(options: ConditionFieldOptions): BlockField {
  ...
  let rows: Condition[] = parseConditions(options.value) ?? [];
  ...
  const state = advanced("condition", options.placeholder, fire, (text) => {
    const read = parseConditions(text);
  ...
  if (options.value.trim() !== "" && parseConditions(options.value) === null) {
    state.setRaw(true, options.value);
    state.note.hidden = false;
  }

  return {
    el: builderRow(options.label, options.doc, state.box).row,
...
export function effectField(options: EffectFieldOptions): EffectField {
  ...
  let lines: TooltipLine[] = (parseEffectLines(options.value) ?? []).map((key) => ({
    key,
  ...
  const state = advanced("effect", options.placeholder, fire, (text) => {
    const read = parseEffectLines(text);
    if (read === null) return false;
    lines = read.map((key) => ({ key, text: options.locOf(key) ?? "" }));
    paint();
    return true;
  });
  state.note.textContent =
  ...
  paint();
  if (options.value.trim() !== "" && parseEffectLines(options.value) === null) {
    state.setRaw(true, options.value);
    state.note.hidden = false;
  }
...
export function chanceField(options: ChanceFieldOptions): BlockField {
  ...
  box.append(input);
  const state = advanced("chance", `{ value = ${options.placeholder} }`, fire, (text) => {
    const value = parseChanceValue(text);
    if (value === null) return false;
    input.value = String(value);
    return true;
  });
  state.note.textContent = "This block weighs the choice with more than a number, so it stays script.";
  state.body.append(box);
  const { row, label } = builderRow(options.label, options.doc, state.box);
  // The label is the drag handle, so the input is only ever typed in.
  scrubbable(input, { step: 1, handle: label, onChange: () => undefined, onCommit: fire });
  if (options.value.trim() !== "" && parsed === null) {
    state.setRaw(true, options.value);
    state.note.hidden = false;
  }
  return {
