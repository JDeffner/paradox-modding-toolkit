...
describe("compatch filesystem and output", () => {
  ...
  it("updates only the original mod while protecting both game versions and links", async () => {
    session.mode = "game-update";
    session.roots.base = path.join(root, "base");
    session.output = session.roots.A!;
    await expect(validateRoots(session)).resolves.toBeUndefined();
    await expect(validateRoots({ ...session, output: session.roots.B! })).rejects.toThrow("original Mod");
    await expect(validateRoots({ ...session, protectedRoots: [session.output] })).rejects.toThrow("separate");
    await expect(
      validateRoots({ ...session, roots: { ...session.roots, base: session.roots.B } })
    ).rejects.toThrow("non-overlapping");
    await fs.symlink(session.roots.B!, path.join(session.output, "linked"), "junction");
    await expect(resultPath(session, "linked/file.txt")).rejects.toThrow("link");
  });
